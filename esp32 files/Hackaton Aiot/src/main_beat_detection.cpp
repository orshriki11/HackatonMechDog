...
// #define SOUND_REFERENCE_PIN 1 // Or whatever pin you are using
...
//     int k = analogRead(A0); // Change to the pin you're using

//     currentMinimum = min(currentMinimum, k);
//     currentMaximum = max(currentMaximum, k);
//     currentAverage += k;

//     k -= 2048; // center to signed 12-bit range
//     k <<= 4;   // scale up to 16-bit signed int
//     k <<= FreqGainFactorBits;

//     fht_input[i] = k;
//     delayMicroseconds(50); // adjust as needed for sample rate
//   }
